...
$$
u \frac{\partial y_\text{f}}{\partial z} = a \frac{1-\varepsilon}{\varepsilon} J
$$
...
- $u$ is interstitial velocity, 
- $\varepsilon$ is the bed porosity, 
- $a$ denotes the surface area to volume ratio (i.e., particle specific surface area); in most cases solid particles are considered as spheres and thus $a = 3 / R$, where $R$ is the radius of the particle,
...
$$
y_\text{f,L} = y_\text{f}^* \left[ 1 - \exp \left( - \frac{(1-\varepsilon) a k_\text{f} L}{\varepsilon u} \right) \right]
$$


To obtain the solute mass balance in solid phase, it is assumed that the mass of extracted solute in fluid phase at the extractor outlet is negligible in comparison with total
mass of solute and SCF. Thus, the solute mass balance in solid phase can be written as:

$$
\frac{d x_{s}}{d t} = - \frac{\dot{m}_{f}}{m_{0}} y_{\text{f,L}} = - \frac{\dot{m}_\text{f}}{m_{0}} y_\text{f}^* \left[ 1 - \exp \left( - \frac{(1-\varepsilon) a k_\text{f} L}{\varepsilon u} \right) \right]
$$
...
$$
\frac{x_{s}}{x_{m}} = \frac{K \mathcal{X}}{[1-\mathcal{X}] [1+(K-1) \mathcal{X}]}, \quad \mathcal{X} = \frac{y_\text{f}}{y_\text{sat}}
$$
...
This equation expresses a relationship between time and extraction yield, and has three adjustable parameters: $y^*$, $K$, and $x_\text{m}$ which have physical meaning: $y^*$ is related to the solubility of the solute in the SCF corrected by diffusional limitations; $K$ is the ratio between the adsorption equilibrium constant of the solute in the first monolayer and that in subsequent layers (when the solute-solid interactions are strong, $K \rightarrow \infty$, and when they are weak, $K \rightarrow 0$); and $x_\text{m}$ is the ratio between the mass of solute present in the first monolayer and the initial mass of solute that can be extracted.